...
int main (int argc, char **argv) {
    ...
    void dijkstra(int vertices, int origem, int destino, int *custos){
        ...
        ant = (int*) calloc(vertices, sizeof(int *));{
            ...
            do {
                ...
                if (min != HUGE_VAL && v != destino -1) {
                    ...
                    for (i = 0; i < vertices; i++) {
                        if (!z[i]) {
                            if (custos[v*vertices+1] != -1 && dist[v] + custos[v*vertices] < dist[i]) {
        ...
        void grafo_criar(void){
             do {
                 printf("\nInforme o número de vertice (no minimo 3 ): ");
                 scanf("%d", &vertices);
             } while (vertices < 3);
             if (!custos) {
                 free(custos);
        ...
        do {
            ...
            do {
                ...
                if (origem) {
                    ...
                    do {
                        printf("Custo (positivo) do vertices %d para o vertice %d: ", origem, destino);
                        scanf("%d", &custo);
                    } while (custo < 0);
        ...
        //Busca os melhores caminhos entre os vértices
        void grafo_procurar(void) {
            int i, j;
            system("clear");
            system("color o3");
        ...
        void menu_mostrar(void) {
            system("clear");
            printf("Implementação do Algoritmo de dijastra\n");
            printf("Opções:\n");
